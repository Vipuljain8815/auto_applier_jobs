...
import pyautogui
import time
def mock_alert(text='', title='', button='OK', *args, **kwargs):
    text_str = str(text)
    print(f"\n[ALERT: {title}] {text_str}\n")
    if "Login" in str(title) or "Login" in text_str:
        print("Waiting 60 seconds for you to login manually in the Chrome window...")
        time.sleep(60)
    elif "Help Needed" in str(title) or "Confirm" in str(title):
        print("Waiting 60 seconds for you to manually answer the question in Chrome...")
        time.sleep(60)
    return button
def mock_confirm(text='', title='', buttons=['OK', 'Cancel'], *args, **kwargs):
    print(f"\n[CONFIRM: {title}] {text}\n")
    if "error" in text.lower() or "filter" in text.lower():
        print("Waiting 15 seconds for you to manually intervene...")
        time.sleep(15)
    return buttons[0]
...
from config.secrets import use_AI, linkedin_email_or_phone as email, linkedin_password as password, ai_provider
...
def login_LN() -> None:
    ...
    * Tries to login using given `email` and `password` from `secrets.py`
    * If failed, tries to login using saved LinkedIn profile button if available
    * If both failed, asks user to login manually
    '''
    # Find the email or phone and password fields and fill them with user credentials
    driver.get("https://www.linkedin.com/login")
    if email == "email@example.com" and password == "example_password":
        pyautogui.alert("User did not configure email or phone and password in secrets.py, hence can't login automatically! Please login manually!", "Login Manually","Okay")
        print_lg("User did not configure email or phone and password in secrets.py, hence can't login automatically! Please login manually!")
        manual_login_retry(is_logged_in_LN, 2)
        return
    try:
        wait.until(EC.presence_of_element_located((By.LINK_TEXT, "Forgot password?")))
        
        # Try finding email or phone field
        try:
            text_input_by_CSS(driver, "input#username, input#session_key, input[name='session_key' i], input[type='text' i], input[type='email' i], input[autocomplete='username' i]", email, 5)
        except Exception as e:
            print_lg(f"Couldn't find email or phone field. Error: {type(e).__name__}")
            try:
                with open("login_error_page.html", "w", encoding="utf-8") as f: f.write(driver.page_source)
            except: pass
        try:
            text_input_by_CSS(driver, "input#password, input[name='session_password' i], input[type='password' i]", password, 5)
        except Exception as e:
        ...
        try:
            ...
            print_lg(f"Failed to click Sign In: {type(btn_e).__name__}")
...
def apply_filters() -> None:
    ...
    try:
        ...
        wait.until(EC.presence_of_element_located((By.XPATH, '//button[contains(@aria-label, "All filters") or contains(., "All filters")]'))).click()
...
def get_job_main_details(job: WebElement, blacklisted_companies: set, rejected_jobs: set) -> tuple[str, str, str, str, str, bool]:
    ...
    * skip: A boolean flag to skip this job
    ...
    try:
        ...
        print_lg(f"Failed to extract job details, element might be stale. Skipping job.")
    ...
    for Question in all_questions:
        ...
        if text: 
            ...
            if not prev_answer or overwrite_previous_answers:
                ...
                elif ('hear' in label or 'come across' in label) and 'this' in label and ('job' in label or 'position' in label): answer = "https://github.com/Vipuljain8815/auto_applier_jobs"
...
def failed_job(job_id: str, job_link: str, resume: str, date_listed, error: str, exception: Exception, application_link: str, screenshot_name: str) -> None:
    ...
    try:
        with open(failed_file_name, 'a', newline='', encoding='utf-8') as file:
            ...
            writer.writerow({'Job ID':truncate_for_csv(job_id), 'Job Link':truncate_for_csv(job_link), 'Date Tried':datetime.now(), 'Assumed Reason':truncate_for_csv(error)})
            file.close()
    except Exception as e:
...
def submitted_jobs(job_id: str, title: str, company: str, work_location: str, work_style: str, description: str, experience_required: int | Literal['Unknown', 'Error in extraction'], 
    ...
    try:
        with open(file_name, mode='a', newline='', encoding='utf-8') as csv_file:
            ...
            writer.writerow({'Job ID':truncate_for_csv(job_id), 'Title':truncate_for_csv(title), 'Company':truncate_for_csv(company),
                                'Job Link':truncate_for_csv(job_link), 'External Job link':truncate_for_csv(application_link), 'Date Applied':truncate_for_csv(date_applied)})
        csv_file.close()
    except Exception as e:
...
def log_website_job(job_id: str, title: str, company: str, job_link: str) -> None:
    '''
    Function to create or update the Website jobs CSV file
    '''
    try:
        existing_links = set()
        try:
        ...
        if job_link.strip() in existing_links:
        ...
        with open("company_website_jobs.csv", mode='a', newline='', encoding='utf-8') as csv_file:
            ...
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            if csv_file.tell() == 0: writer.writeheader()
            writer.writerow({'Job ID':truncate_for_csv(job_id), 'Title':truncate_for_csv(title), 'Company':truncate_for_csv(company), 'Job Link':truncate_for_csv(job_link)})
        csv_file.close()
    except Exception as e:
        print_lg("Failed to update website jobs list!", e)
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    # DISABLED: To prevent opening external website tabs.
                    if is_easy_apply:
                        try: 
                            try:
                                errored = ""
...
def main() -> None:
    ...
    try:
        ...
        if not is_logged_in_LN(): 
            login_LN()
            if not is_logged_in_LN():
                raise Exception("Login failed. Exiting script before applying to jobs.")
        ...
        quotes = choice([
            "Never quit. You're one step closer than before. - Vipul jain", 
            "All the best with your future interviews, you've got this. - Vipul jain", 
            "Keep up with the progress. You got this. - Vipul jain", 
            "If you're tired, learn to take rest but never give up. - Vipul jain",
